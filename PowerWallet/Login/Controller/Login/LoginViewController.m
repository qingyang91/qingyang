...
@interface LoginViewController ()<KDTextfieldDelegate>
...
- (void)btnClick
{
    ...
    [self.request checkPhoneNumberWithDict:@{@"phone":self.textField.text,@"type":@"0"} onSuccess:^(NSDictionary *dictResult) {
        ...
        //代表手机号为注册,并发送验证码
        RegistViewController *registerVC = [[RegistViewController alloc] initWithPageType:registerSec];
        ...
        //200 该手机号以注册
        if (code == 1000) {
            LoginSecViewController *secVC = [LoginSecViewController new];
