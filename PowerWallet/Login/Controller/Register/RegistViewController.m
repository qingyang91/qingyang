    ...
    if (self.type == registerSec) {
        ...
        if ([weakSelf.getCodeBtn.titleLabel.text isEqualToString:@"重新发送"]||[weakSelf.getCodeBtn.titleLabel.text isEqualToString:@"发送验证码"]) {
            [UIButton startRunSecond:_getCodeBtn stringFormat:@"%@秒后重试" finishBlock:^{
                ...
                [weakSelf.getCodeBtn setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
            }];
        }
...
{
    __weak typeof(self) weakSelf = self;
    self.graphTextfield = [[UITextField alloc] init];
    self.graphTextfield.font = [UIFont systemFontOfSize:15.0];
    self.graphTextfield.textColor = [UIColor colorWithHex:0x333333];
    self.graphTextfield.placeholder = @"请输入图形验证码";
    self.graphTextfield.translatesAutoresizingMaskIntoConstraints = NO;
    [self.view addSubview:self.graphTextfield];
    [self.graphTextfield mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    _graphTextfield.layer.borderColor = [UIColor orangeColor].CGColor;
    ...
    [_getCodeBtn setTitleColor:Color_Response forState:UIControlStateNormal];
    [_getCodeBtn setTitle:@"发送验证码" forState:UIControlStateNormal];
    _getCodeBtn.layer.masksToBounds = YES;
    _getCodeBtn.layer.borderColor = [UIColor orangeColor].CGColor;
    ...
    _inviteTextfield.layer.borderColor = [UIColor orangeColor].CGColor;
    ...
    _commitBtn.backgroundColor = [UIColor colorWithHex:0xD8D8D8];
