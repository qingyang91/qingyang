...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.navigationItem.title = @"待确认收钱借条";
    [self loadData];
}
...
- (void)creatUI{
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"ConfirmListViewCell" bundle:nil] forCellReuseIdentifier:@"ConfirmListViewCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ConfirmListViewCell *cell = [_tableView dequeueReusableCellWithIdentifier:@"ConfirmListViewCell" forIndexPath:indexPath];
    ...
    cell.xiaoBtn.layer.cornerRadius = 5;
    cell.xiaoBtn.layer.masksToBounds = YES;
    [cell.xiaoBtn addTarget:self action:@selector(xiaoClickWithBtn:) forControlEvents:UIControlEventTouchUpInside];
    cell.xiaoBtn.tag = [model.lendID integerValue];
    cell.feedBtn.layer.cornerRadius = 5;
    cell.feedBtn.layer.masksToBounds = YES;
    cell.feedBtn.tag = [model.lendID integerValue]+1;
    [cell.feedBtn addTarget:self action:@selector(feedClickWithBtn:) forControlEvents:UIControlEventTouchUpInside];
    return cell;
}
