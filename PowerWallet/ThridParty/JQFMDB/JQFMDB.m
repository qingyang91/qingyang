        ...
        while ([set next]) {
            ...
            for (NSString *key in dic) {
                ...
                if ([dic[key] isEqualToString:SQL_TEXT]) {
                    ...
                    resultDic = [NSMutableDictionary dictionaryWithObject:@([set longLongIntForColumn:key]) forKey:key];
                } else if ([dic[key] isEqualToString:SQL_REAL]) {
                    resultDic = [NSMutableDictionary dictionaryWithObject:[NSNumber numberWithDouble:[set doubleForColumn:key]] forKey:key];
                } else if ([dic[key] isEqualToString:SQL_BLOB]) {
                    id value = [set dataForColumn:key];
                    if (value)
                        resultDic = [NSMutableDictionary dictionaryWithObject:value forKey:key];
                }
            }
