...
- (UIScrollView *)ccpScrollView {
    ...
    if (_ccpScrollView == nil) {
        ...
        _ccpScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH-128, self.frame.size.height)];
...
//        titleLabel.backgroundColor = [UIColor redColor];
        ...
        titleLabel.backgroundColor = [UIColor colorWithWhite:1 alpha:0.2];//[UIColor colorWithHex:0xffffff alpha:0.2];
...
- (void)nextLabel {
    ...
    CGFloat perHeight = iPhone4 ? 5*WIDTHRADIUS : 28*WIDTHRADIUS;
    if (!(int)oldPoint.y % (int)perHeight) {
        oldPoint.y = (int)(oldPoint.y / perHeight) * perHeight;
    }
...
- (void)addTimer{
    ...
    self.timer = [NSTimer timerWithTimeInterval:5.f target:self selector:@selector(nextLabel) userInfo:nil repeats:YES];
