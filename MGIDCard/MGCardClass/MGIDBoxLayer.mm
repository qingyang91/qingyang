...
- (void)drawBox:(CGRect)box Context:(CGContextRef)ctx{
    ...
    UIImageView * backImage = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"Verify_cardback"]];
    backImage.frame = CGRectMake(box.origin.x, box.origin.y, box.size.width, box.size.height);
